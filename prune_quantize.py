...
def sensitivity_prune(model, cfg, val_loader):
    """
    ...
    sens = np.array(list(sens.values())) # Assuming evaluate_sensitivity returns a dict of sensitivities
    ...
    if (max_s - min_s) == 0:
        ...
        ratios = np.ones_like(sens) * cfg['pruning'].get('amount', 0.2) # Use a default amount
    else:
        ratios = 1 - (sens - min_s) / (max_s - min_s)  # invert: high sensitivity→low prune
    ...
    if len(prunable_modules) != len(ratios):
    ...
    for i, (name, module) in enumerate(prunable_modules):
        ...
        if r > cfg['pruning'].get('min_sparsity', 0.0): # Use a default min_sparsity if not in config
            logger.info(f"Applying structured pruning to {name} with amount={float(r):.4f}")
            apply_structured_pruning(module, amount=float(r),
                                     n=cfg['pruning']['n_groups']) # n_groups should be in config
...
def run_pruning_and_quantization(cfg: Dict[str, Any]):
    ...
    if cfg['training']['pruning']['enabled']:
        ...
        if cfg['training']['pruning']['use_sensitivity']:
            ...
            model = sensitivity_prune(model, cfg, val_loader)
        else:
...
if __name__ == "__main__":
    ...
    #     n_groups: 1 # Added for sensitivity_prune, assuming 1 for unstructured or specific group size
