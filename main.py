...
# We import the global CONFIG object and the load_config function from config.py
from config import CONFIG, DEVICE, load_config, IMAGENET_MEAN, IMAGENET_STD, \
...
def main():
    ...
    CONFIG = load_config() # Load default or from config.yaml if it exists

    # Set logging level from config
    logger.setLevel(CONFIG['debug']['log_level'])

    # Create checkpoints directory if it doesn't exist
    os.makedirs(CONFIG['debug']['save_model_checkpoints'], exist_ok=True)
    logger.info(f"Checkpoints will be saved to: {CONFIG['debug']['save_model_checkpoints']}")

    # Initialize Weights & Biases if enabled
    if CONFIG['HAS_WANDB'] and CONFIG['training'].get('use_wandb', False):
        wandb.init(project="bongard_solver", config=CONFIG)
        logger.info("Weights & Biases initialized.")
    else:
        logger.info("Weights & Biases is disabled or not found.")

    # 2. YOLO Object Detector Fine-tuning (if enabled)
    if CONFIG['object_detector']['fine_tune_yolo']:
        logger.info("Initiating YOLO fine-tuning.")
        best_yolo_weights_path = fine_tune_yolo(CONFIG)
        if best_yolo_weights_path:
            CONFIG['object_detector']['yolo_pretrained'] = best_yolo_weights_path
            ...
            # CONFIG['object_detector']['use_yolo'] = False
    ...
    if CONFIG['pruning']['enabled'] or CONFIG['quantization']['qat'] or CONFIG['quantization']['ptq']:
        logger.info("Initiating Pruning and Quantization pipeline.")
        run_pruning_and_quantization(CONFIG)
    ...
    if HAS_SCENE_GRAPH_BUILDER and HAS_YOLO and HAS_SAM:
        ...
        demo_image = np.zeros((CONFIG['data']['image_size'], CONFIG['data']['image_size'], 3), dtype=np.uint8)
        ...
        try:
            yolo_model, sam_predictor = load_yolo_and_sam_models(CONFIG)
            ...
            detected_bboxes, detected_masks, _ = detect_and_segment_image(
                demo_image, yolo_model, sam_predictor, CONFIG
            )
            ...
            if detected_bboxes:
                ...
                dummy_attribute_logits = {
                    'shape': torch.randn(len(detected_bboxes), CONFIG['model']['attribute_classifier_config']['shape']),
                    'color': torch.randn(len(detected_bboxes), CONFIG['model']['attribute_classifier_config']['color']),
                    'fill': torch.randn(len(detected_bboxes), CONFIG['model']['attribute_classifier_config']['fill']),
                    'size': torch.randn(len(detected_bboxes), CONFIG['model']['attribute_classifier_config']['size']),
                    'orientation': torch.randn(len(detected_bboxes), CONFIG['model']['attribute_classifier_config']['orientation']),
                    'texture': torch.randn(len(detected_bboxes), CONFIG['model']['attribute_classifier_config']['texture']),
                }
                ...
                dummy_relation_logits = torch.randn(num_edges, CONFIG['model']['relation_gnn_config']['num_relations'])

                # Build the scene graph
                scene_graph_builder = SceneGraphBuilder(CONFIG)
