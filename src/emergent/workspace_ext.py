...
try:
    ...
    CONFIG = {'model': {'attribute_classifier_config': {'shape': 5, 'color': 7}, 'image_size': [64,64]}, 'replay': {'buffer_capacity': 30000}}  # Dummy config
# Import primitive_extractor for feature extraction with confidence
try:
    from src.perception.primitive_extractor import extract_shape_conf, extract_fill_conf, extract_cnn_features, _load_cnn_model, attribute_maps_inv # Import attribute_maps_inv
    ...
    attribute_maps_inv = { # Dummy if not imported
...
try:
    ...
    class SceneGraphBuilder:
        ...
        def get_object_image(self, obj_id: str) -> Optional[Any]:
            ...
            # Return a blank image matching the expected size from config
            img_size = CONFIG['model']['image_size'][0] if 'model' in CONFIG and 'image_size' in CONFIG['model'] else 64
            return Image.new('RGB', (img_size, img_size), color=(random.randint(0,255), random.randint(0,255), random.randint(0,255)))
...
class Workspace:
    ...
    def primitive_feature(self, obj_id: str, feat_type: str) -> Tuple[Any, float]:
        ...
        # Push crop + pseudo-label for continuous fine-tuning
        ...
        if obj_image is not None and HAS_PRIMITIVE_EXTRACTOR and _load_cnn_model():
            try:
                # Get the class names from the inverse attribute maps
                class_names_for_attr = list(attribute_maps_inv.get(feat_type, {}).keys()) # Get keys from inverse map
                
                if val in class_names_for_attr: # Check if the value exists in the class names
                    label_idx = class_names_for_attr.index(val) # Get the index
...
if __name__ == '__main__':
    ...
    dummy_config = {
        'model': {
            ...
            'image_size': [64, 64] # Added image_size for dummy SG
        },
        'data': {'image_size': [64, 64]},
        'slipnet_config': {'activation_threshold': 0.1},
        'replay': {'buffer_capacity': 100} # Small capacity for testing
    }
