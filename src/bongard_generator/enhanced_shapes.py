...
def create_nested_shape(draw: ImageDraw.Draw, cx: int, cy: int, size: int,
    ...
    # Ensure fill_color and outline_color are tuples or ints
    if isinstance(fill_color, str):
        try:
            fill_color = ImageColor.getrgb(fill_color)
        except Exception:
            fill_color = (0, 0, 0)
    elif not (isinstance(fill_color, tuple) or isinstance(fill_color, int)):
        fill_color = (0, 0, 0)
    if isinstance(outline_color, str):
        try:
            ...
            outline_color = (0, 0, 0)
    elif not (isinstance(outline_color, tuple) or isinstance(outline_color, int)):
        outline_color = (0, 0, 0)
    # Draw outer circle
...
def draw_dashed_line(draw: ImageDraw.Draw, start: Tuple[int, int], end: Tuple[int, int],
    ...
    # Ensure color is a tuple or int
    if isinstance(color, str):
        try:
            color = ImageColor.getrgb(color)
        except Exception:
            color = (0, 0, 0)
    elif not (isinstance(color, tuple) or isinstance(color, int)):
        color = (0, 0, 0)
    x1, y1 = start
    x2, y2 = end
...
def draw_dashed_polygon(draw: ImageDraw.Draw, vertices: List[Tuple[float, float]],
    ...
    for i in range(len(vertices)):
        ...
        draw_dashed_line(draw, start_int, end_int, dash_len, gap_len, color, width)
...
def draw_dotted_polygon(draw: ImageDraw.Draw, vertices: List[Tuple[float, float]],
    ...
    for i in range(len(vertices)):
        ...
        start_int = (int(round(start[0])), int(round(start[1])))
        end_int = (int(round(end[0])), int(round(end[1])))
        draw_dotted_line(draw, start_int, end_int, dot_spacing, dot_size, color)
