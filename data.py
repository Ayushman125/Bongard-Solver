...
# Import ReplayBuffer (assuming it's in replay_buffer.py)
from replay_buffer import KnowledgeReplayBuffer

logger = logging.getLogger(__name__)


# --- Utility Functions (moved here for direct access) ---
    ...
    def __getitem__(self, idx: int) -> Tuple[np.ndarray, np.ndarray, int, bytes, bytes, float, np.ndarray, np.ndarray, int, List[np.ndarray], List[int], List[bytes], int, Optional[int], Optional[float]]:
        ...
        Returns:
            Tuple: (query_image_view1_np, query_image_view2_np, query_label,
                    ...
                    support_images_np_list, support_labels_list, support_gt_json_bytes_list,
...
class BongardAugmentationPipeline(Pipeline):
    ...
    def _apply_transforms(self, imgs: types.Tensor) -> types.Tensor:
        """
        Applies a sequence of DALI transforms to a batch of images.
        """
        # Decode (if input was encoded, but external_source already provides raw)
        # decoded = fn.decoders.image(imgs, device="mixed", output_type=types.RGB) # Not needed if input is HWC numpy
        
        # Cast to float for augmentations
        imgs = fn.cast(imgs, dtype=types.FLOAT, device=self.dali_op_device)

        # 1. Resize to target dimensions
        # The `self.image_size` here is the *initial* size. For curriculum, it needs to be dynamic.
        # However, DALI pipelines are static graphs. To make `resize_x/y` dynamic,
        # we'd need to recreate the pipeline or use `fn.resize` with dynamic inputs.
        # For now, `self.image_size` will be fixed after pipeline build.
        # If curriculum image size changes, the pipeline would need to be rebuilt.
        # A simpler approach for curriculum with DALI is to have multiple pipelines
        # or handle resizing outside DALI for curriculum.
        # For this implementation, the `image_size` passed to pipeline init is the
        # `end_image_size` from the config, meaning DALI will always resize to this fixed size.
        # The `BongardGenerator` will generate images at the `current_image_size` from the sampler,
        # and DALI will then resize these (potentially smaller/larger) images to its fixed target size.
        imgs = fn.resize(imgs,
...
class BongardGenerator:
    ...
    def _apply_rule_constraints_for_generation(self,
        ...
        if target_rule.name == "all_circles":
        ...
        elif target_rule.name == "all_same_color_red":
            for attrs in modified_attrs:
                attrs['color'] = 'red'
        ...
        elif target_rule.name == "exists_large_square":
        ...
        elif target_rule.name == "small_above_large":
