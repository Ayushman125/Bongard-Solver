...
try:
    ...
    class SymbolicEngine:
        ...
        def evaluate_scene_graph_against_rule(self, scene_graph: Dict[str, Any], rule: Any) -> bool:
            ...
            if isinstance(rule, BongardRule) and hasattr(rule, 'is_positive_rule') and rule.is_positive_rule: return True
            if isinstance(rule, BongardRule) and hasattr(rule, 'is_positive_rule') and not rule.is_positive_rule: return False
...
class GradNorm(nn.Module):
    ...
    def update_weights(self, losses: Dict[str, float], shared_parameters: List[torch.Tensor]):
        """
        ...
        Args:
            losses (Dict[str, float]): A dictionary of scalar loss values for each task.
        ...
        for i, name in enumerate(self.task_names):
            ...
            # Convert scalar float loss to tensor for autograd
            loss_tensor = torch.tensor(losses[name], device=shared_parameters[0].device, requires_grad=True)
...
class CrossEntropyWithConfidence(nn.Module):
    """
