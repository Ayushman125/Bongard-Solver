...
class KnowledgeReplayBuffer:
    ...
    def sample(self, batch_size: int) -> Tuple[List[int], List[int], List[float]]:
        ...
        # Anneal beta
        # This annealing should ideally be tied to global steps/epochs, not per sample.
        # It's better to update beta in `set_epoch` in `CurriculumSampler`.
    ...
    def update_priorities(self, tree_indices: List[int], losses: List[float]):
        """
        Updates priorities of sampled experiences based on their new losses.
        """
        if len(tree_indices) != len(losses):
            logger.error(f"Mismatch in lengths: tree_indices ({len(tree_indices)}) vs losses ({len(losses)})")
            return
